...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <p class="caution">※"有" を選択した場合は情報を入力してください。"有" を選択した際でも、入力がなかった場合は、閲覧する際に "無し"として表示されます。</p>
        <div id="list">
            <table>
                <tr>
                    <td><span>*</span>スキー場名</td> 
                    <td>
                        <input class="input_list" name="ski_area[place_name]" placeher="例１：札幌国際スキー場　例２：ルスツリゾート" value="{{ old('ski_area.place_name', $ski_area->place_name) }}">
                ...
                <tr>
                    <td>ホームページ</td> 
                    <td>
                        <input class="input_list" name="ski_area[home_page]" placeholder="例：https://www.sapporo-kokusai.jp/" value="{{ old('ski_area.home_page', $ski_area->home_page) }}">
                ...
                <tr>
                    ...
                    <td>
                        <input class="input_list" name="ski_area[zip_code]" placeholder="例：010-0012" value="{{ old('ski_area.zip_code', $ski_area->zip_code) }}">
                ...
                <tr>
                    ...
                    <td>
                        <input class="input_list" name="ski_area[prefecture]" placeholder="例：北海道" value="{{ old('ski_area.prefecture', $ski_area->prefecture) }}">
                ...
                <tr>
                    ...
                    <td>
                        <input class="input_list" name="ski_area[city]" placeholder="例：南区" value="{{ old('ski_area.city', $ski_area->city) }}">
                ...
                <tr>
                    ...
                    <td>
                        <input class="input_list" name="ski_area[phone_number]" placeholder="例：011-598-4519" value="{{ old('ski_area.phone_number', $ski_area->phone_number) }}">
                ...
                <tr>
                    <td>シーズン期間</td> 
                    <td>
                        <input class="input_list" name="ski_area[season]" placeholder="例：2022年11月18日（金） ~ 2023年5月7日（日）" value="{{ old('ski_area.season', $ski_area->season) }}">
                ...
                <tr>
                    <td>リフト料金</td> 
                    <td>
                        <textarea class="normal_textarea" name="ski_area[lift_ticket]" placeholder="例：1日券：4000円¥n4時間券：3000円">{{ old('ski_area.lift_ticket', $ski_area->lift_ticket) }}</textarea>
                ...
                <tr>
                    <td>駐車場</td> 
                    <td>
                        @if ((old('ski_area.parking_lot')===null) && $ski_area->parking_lot === null)  
                        ...
                        @else
                            ...
                            <textarea id=8 class="detail" style="display: block" name="ski_area[parking_lot]" placeholder="例：第一駐車場（800台）" >{{ old('ski_area.parking_lot', $ski_area->parking_lot) }}</textarea>
                            <p class=error>{{ $errors->first('ski_area.parking_lot') }}</p>
                        @endif
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        ...
                        @else
                            ...
                            <textarea id=7 class="detail" style="display: block" name="ski_area[kids_park]" placeholder="例：場所：センターハウス横¥nソリー使用可能">{{ old('ski_area.kids_park', $ski_area->kids_park) }}</textarea>
                            <p class=error>{{ $errors->first('ski_area.kids_park') }}</p>
                        @endif
                    </td>
                </tr>
                <tr>
                    <td>ナイター</td> 
                    <td>
                        @if ((old('ski_area.evening_hours')===null) && $ski_area->evening_hours===null)
                        ...
                        @else
                            ...
                            <textarea id=1 class="detail" style="display: block" name="ski_area[evening_hours]" placeholder="例：17:00 ~ 20:00">{{ old('ski_area.evening_hours', $ski_area->evening_hours) }}</textarea>
                            <p class=error>{{ $errors->first('ski_area.evening_hours') }}</p>
                        @endif
                    </td>
                </tr>
                <tr>
                    <td>レッスン</td> 
                    <td>
                        @if ((old('ski_area.lesson')===null) && $ski_area->lesson===null)
                        ...
                        @else
                            ...
                            <textarea id=2 class="detail" style="display: block" name="ski_area[lesson]" placeholder="例：３日集中レッスン、キッズ・ジュニアレッスン、レベル別レッスン、プライベートレッスン">{{ old('ski_area.lesson', $ski_area->lesson) }}</textarea>
                            <p class=error>{{ $errors->first('ski_area.lesson') }}
                        @endif
                    </td>
                </tr>
                    ...
                    <td>レストラン</td> 
                     <td>
                        ...
                        @else
                            ...
                            <textarea id=3 class="detail" style="display: block" name="ski_area[restaurant]" placeholder="スキー場内に６つ有り">{{ old('ski_area.restaurant', $ski_area->restaurant) }}</textarea>
                            <p class=error>{{ $errors->first('ski_area.restaurant') }}</p>
                        @endif
                    </td>
                </tr>
                <tr>
                    <td>温泉</td> 
                    <td>
                        @if ((old('ski_area.spa')===null) && $ski_area->spa===null)
                        ...
                        @else
                            ...
                            <textarea id=4 class="detail" style="display: block" name="ski_area[spa]" placeholder="例：◯◯温泉 （スキー場から1km ）">{{ old('ski_area.spa', $ski_area->spa) }}</textarea>
                            <p class=error>{{ $errors->first('ski_area.spa') }}
                        @endif
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        ...
                        @else
                            ...
                            <textarea id=5 class="detail" style="display: block" name="ski_area[hotel]" placeholder="◯◯ホテル（スキー場から15km ）">{{ old('ski_area.hotel', $ski_area->hotel) }}</textarea>
                            <p class=error>{{ $errors->first('ski_area.hotel') }}</p>
                        @endif
                        </td>
                </tr>
